...
def genModelPreds_biasOC_2LR_SW(data, lr_pre, lr_post, d, p0):
...
def optimize2LR_sft_fixedSM_posNegOC_p0_switch(params, choices, outcomes, smax=1):
    ...
    mod=genModelPreds_biasOC_2LR_SW(outcomes,lr_pre,lr_post,d,p0)
...
for PID in pids:
    ...
    all_lrs_nll = np.inf
    for lrs, d, p0 in zip(lrRange,drange,p0Range):
    ...
    allLrnSumm.loc[PID,'subj2LR_pre']=np.exp(lrs_pre_val)/(np.exp(lrs_pre_val)+1)
    allLrnSumm.loc[PID,'subj2LR_post']=np.exp(lrs_post_val)/(np.exp(lrs_post_val)+1)
