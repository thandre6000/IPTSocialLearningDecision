...
lrnEstArr=data['rwlrnEstArr']
lrnOCArr=data['rwlrnOCArr']
lrnSumm=data['rwlrnSumm']
...
for PID in pids:
    ...
    rwestDat=lrnEstArr.loc[(lrnEstArr.PID==PID)].transpose().dropna().drop('PID').astype(int)
    rwestDat.columns=['ch_{:02d}'.format(i) for i in rwestDat.columns+1]
    rwocDat=lrnOCArr.loc[(lrnOCArr.PID==PID)].transpose().dropna().drop('PID').astype(int)
    rwlrs_nll=np.inf
    for lr0 in lrRange:
    ...
    lrnSumm.loc[PID,'subjLR']=np.exp(rwlrs_lr)/(np.exp(rwlrs_lr)+1)
    lrnSumm.loc[PID,'subj2NLL_biasedOC']=rwlrs_nll
    rwchncNLL=np.sum(chanceLikelihood(rwestDat))  
    lrnSumm.loc[PID, 'subj2LRTp_biasedOC']=stats.chi2.sf(2*(rwchncNLL-lrnSumm.loc[PID,'subj2NLL_biasedOC']),1)
...
final_file = "results/1LR_TRUST.csv"
