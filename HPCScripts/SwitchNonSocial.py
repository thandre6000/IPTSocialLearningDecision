...
for PID in pids:
    ...
    for lrs, p0, d in zip(lrRange,p0Range,drange):
        result_s=sp.optimize.minimize(optimize2LR_sft_fixedSM_posNegOC_p0_switch, (lrs,lrs,p0,d),(rwestDat, rwocDat), bounds=((-10,10),(-10,10),(-10,10),(-10,10)))
    ...
    rwlrnSumm.loc[PID,'subj2LR_pre']=np.exp(lrs_pre_val)/(np.exp(lrs_pre_val)+1)
    rwlrnSumm.loc[PID,'subj2LR_post']=np.exp(lrs_post_val)/(np.exp(lrs_post_val)+1)
    rwlrnSumm.loc[PID,'subj2p0_4prm']=np.exp(lrs_p0)/(np.exp(lrs_p0)+1)*2 - 1
    rwlrnSumm.loc[PID,'subj2D_4prm']=np.exp(lrs_d)/(np.exp(lrs_d)+1)
...
final_file = "results/BiasOut1LR_LOT.csv"
