...
for PID in pids:
    ...
    for lrs, p0s in zip(lrRange, p0Range):
        result_s=sp.optimize.minimize(optimize2LR_sft_fixedSM_p0, (lrs,lrs,p0s),(estDat, ocDat), bounds=((-10,10),(-10,10),(-10,10)))
    ...
    lrnSumm.loc[PID,'subj2p0_3prm']= (np.exp(lrs_p0)/(np.exp(lrs_p0)+1))
    ...
    lrnSumm.loc[PID, 'subj2LRTp_biasedOC']=stats.chi2.sf(2*(chncNLL-lrnSumm.loc[PID,'subj2NLL_biasedOC']),3)
...
final_file = "results/2LRPosNegOCp0NonSocial_TRUST.csv"
