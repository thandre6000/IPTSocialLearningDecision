...
for PID in pids:
    ...
    for lrs, ds, p0s in zip(lrRange,dRange, p0Range):
            result_s=sp.optimize.minimize(optimizeLR_sft_fixedSM_posNegOC_p0, (lrs,ds,p0s),(rwestDat, rwocDat),bounds=((-10,10),(-10,10),(-10,10)))
    ...
    rwlrnSumm.loc[PID,'subjp0_3prm']=np.exp(rwlrs_p0)/(np.exp(rwlrs_p0)+1)*2 - 1
    rwlrnSumm.loc[PID,'subjNLL_3prm']=rwlrs_nll
    chncNLL=np.sum(chanceLikelihood(rwestDat))
    rwlrnSumm.loc[PID, 'subjLRTp_3prm']=stats.chi2.sf(2*(chncNLL-rwlrnSumm.loc[PID,'subjNLL_3prm']),1)
...
final_file = "results/1LRP0D_LOT.csv"
